...
export default class D4HMembersProvider implements MemberProvider {
  ...
  async getMemberInfo(auth: MemberAuthInfo): Promise<MemberInfo | undefined> {
    await this.initialize();

    const token = process.env.D4H_TOKEN!
    const info = this.tokenFetchInfo[token]?.lookup[this.tokenFetchInfo[token]?.authEmailToD4HId?.[auth.email]]?.memberInfo;
    if (info) {
      return {
        ...info,
      };
  ...
  async refresh(force?: boolean) {
    ...
    try {
      ...
      if (!process.env.D4H_TOKEN) {
        throw new Error('D4H_TOKEN not set');
      }

      await this.refreshMembersForToken(process.env.D4H_TOKEN!, 'Secondary Email');
